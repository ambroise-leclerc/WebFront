...
#ifndef NET_TS_DETAIL_SOCKET_OPS_IPP
#define NET_TS_DETAIL_SOCKET_OPS_IPP
...
#else // defined(NET_TS_WINDOWS) && !defined(NET_TS_WINDOWS_APP)
      // || defined(__CYGWIN__)
  (void)cached;
    ...
    unsigned long scope_id, std::error_code& ec)
{
  ...
  else if (af == NET_TS_OS_DEF(AF_INET6))
  {
    size_t n = 0, b = 0, z = 0;
    while (n < length && b < 16)
    {
  ...
  if (af == NET_TS_OS_DEF(AF_INET))
  {
    ...
    address.v4.sin_family = NET_TS_OS_DEF(AF_INET);
  ...
  else // AF_INET6
  {
    ...
    address.v6.sin6_family = NET_TS_OS_DEF(AF_INET6);
  ...
  if (result != 0 && af == NET_TS_OS_DEF(AF_INET6) && scope_id != 0)
  {
    ...
    if ((!is_link_local && !is_multicast_link_local)
        ...
        snprintf(if_name + 1, (IF_NAMESIZE > 21 ? IF_NAMESIZE : 21), "%lu", scope_id);
    strcat(dest, if_name);
  }
  return result;
#endif // defined(NET_TS_WINDOWS) || defined(__CYGWIN__)
    ...
    unsigned long* scope_id, std::error_code& ec)
{
  ...
  const bool is_v6 = (af == NET_TS_OS_DEF(AF_INET6));
  ...
  if (host == 0 || host[0] == '\0')
  {
    if (hints->ai_flags & AI_PASSIVE)
    {
      ...
      case NET_TS_OS_DEF(AF_INET):
        ...
        search[search_count].family = NET_TS_OS_DEF(AF_INET);
        ++search_count;
        break;
      case NET_TS_OS_DEF(AF_INET6):
        search[search_count].host = "0::0";
        search[search_count].family = NET_TS_OS_DEF(AF_INET6);
        ++search_count;
        break;
      case NET_TS_OS_DEF(AF_UNSPEC):
        search[search_count].host = "0::0";
        search[search_count].family = NET_TS_OS_DEF(AF_INET6);
        ++search_count;
        search[search_count].host = "0.0.0.0";
  ...
  case NET_TS_OS_DEF(AF_INET):
    {
      ...
      if (sinptr == 0)
        return EAI_MEMORY;
      sinptr->sin_family = NET_TS_OS_DEF(AF_INET);
  ...
  case NET_TS_OS_DEF(AF_INET6):
    {
      ...
      if (sin6ptr == 0)
        return EAI_MEMORY;
      sin6ptr->sin6_family = NET_TS_OS_DEF(AF_INET6);
    ...
    const addrinfo_type* hintsp, addrinfo_type** result)
{
  ...
  hints.ai_family = NET_TS_OS_DEF(AF_UNSPEC);
  if (hintsp)
    hints = *hintsp;
  ...
  if (hints.ai_family != NET_TS_OS_DEF(AF_INET6))
  ...
  for (gai_search* sptr = search; sptr < search + search_count; ++sptr)
  {
    ...
    std::error_code ec;
    if (socket_ops::inet_pton(NET_TS_OS_DEF(AF_INET),
          sptr->host, &inaddr, 0, ec) == 1)
    {
      if (hints.ai_family != NET_TS_OS_DEF(AF_UNSPEC)
          && hints.ai_family != NET_TS_OS_DEF(AF_INET))
      {
        freeaddrinfo_emulation(aihead);
        gai_free(canon);
        return EAI_FAMILY;
      }
      if (sptr->family == NET_TS_OS_DEF(AF_INET))
      {
        rc = gai_aistruct(&ainext, &hints, &inaddr, NET_TS_OS_DEF(AF_INET));
        if (rc != 0)
        {
          freeaddrinfo_emulation(aihead);
          gai_free(canon);
    ...
    if (socket_ops::inet_pton(NET_TS_OS_DEF(AF_INET6),
          sptr->host, &in6addr, 0, ec) == 1)
    {
      if (hints.ai_family != NET_TS_OS_DEF(AF_UNSPEC)
          && hints.ai_family != NET_TS_OS_DEF(AF_INET6))
      {
        freeaddrinfo_emulation(aihead);
        gai_free(canon);
        return EAI_FAMILY;
      }
      if (sptr->family == NET_TS_OS_DEF(AF_INET6))
      {
        rc = gai_aistruct(&ainext, &hints, &in6addr,
            NET_TS_OS_DEF(AF_INET6));
        if (rc != 0)
        {
          freeaddrinfo_emulation(aihead);
          gai_free(canon);
...
# else
  ...
  return ec = translate_addrinfo_error(error);
# endif
#elif !defined(NET_TS_HAS_GETADDRINFO)
  int error = getaddrinfo_emulation(host, service, &hints, result);
  return ec = translate_addrinfo_error(error);
#else
  int error = ::getaddrinfo(host, service, &hints, result);
...
#endif // NET_TS_DETAIL_SOCKET_OPS_IPP
