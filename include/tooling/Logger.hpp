...
template<typename... Ts> struct debug { debug(string_view fmt, Ts&&... ts, const srcLoc& l = srcLoc::current()) {
    if (is(Debug)) log(Debug, vformat(fmt, make_format_args(forward<Ts>(ts)...)), l);}
};
template<typename... Ts> debug(string_view, Ts&&...) -> debug<Ts...>;
template<typename... Ts> void error(string_view fmt, Ts&&... ts) { log(Error, vformat(fmt, make_format_args(forward<Ts>(ts)...))); }
template<typename... Ts> void warn(string_view fmt, Ts&&... ts) { log(Warn, vformat(fmt, make_format_args(forward<Ts>(ts)...))); }
template<typename... Ts> void info(string_view fmt, Ts&&... ts) { log(Info, vformat(fmt, make_format_args(forward<Ts>(ts)...))); }
