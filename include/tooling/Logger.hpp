...
namespace {
    ...
    inline static void log(LogType l, string_view text) { out(format(string("[{}] {:%T} | {}"), toChar(l), chrono::system_clock::now(), text));}
...
template<class... Ts> struct debug { debug(string_view fmt, Ts&&... ts, const srcLoc& l = srcLoc::current()) {if (is(Debug)) log(Debug, format(fmt, forward<Ts>(ts)...), l);}};
template<class... Ts> debug(string_view, Ts&&...) -> debug<Ts...>;
template<class... Ts> struct error { error(string_view fmt, Ts&&... ts) { if (is(Error)) log(Error, format(fmt, forward<Ts>(ts)...)); } };
template<class... Ts> error(string_view, Ts&&...) -> error<Ts...>;
template<class... Ts> struct warn { warn(string_view fmt, Ts&&... ts) { if (is(Warn)) log(Warn, format(fmt, forward<Ts>(ts)...)); } };
template<class... Ts> warn(string_view, Ts&&...) -> warn<Ts...>;
template<class... Ts> struct info { info(string_view fmt, Ts&&... ts) { if (is(Info)) log(Info, format(fmt, forward<Ts>(ts)...)); } };
template<class... Ts> info(string_view, Ts&&...) -> info<Ts...>;
