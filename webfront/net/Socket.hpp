...
public:
    ...
    void bind() {
        if (::bind(sock, reinterpret_cast<sockaddr*>(&this->sockAddr), sizeof(this->sockAddr)) < 0)
            throw SocketException("Error on binding");
    }

    void listen() {
        if (::listen(sock, SOMAXCONN) < 0)
            throw SocketException("Error on listen");
    }

    Socket accept() {
        Socket client;
        decltype(IPv::sockAddr) addr;
        socklen_t addrSize = sizeof(IPv::sockAddr);
        client.sock = ::accept(sock, reinterpret_cast<sockaddr*>(&addr), &addrSize);
        if (client.sock < 0)
...
public:
    ...
    void setPort(uint16_t port) noexcept { sockAddr.sin_port = port; }
...
public:
    ...
    void setPort(uint16_t port) noexcept { sockAddr.sin6_port = port; }
...
class ScopeExitGuard {
    std::function<void()> f;
public:
    template<typename Callable> ScopeExitGuard(Callable&& f) : f(std::forward<Callable>(f)) {}
    ~ScopeExitGuard() { try { f(); } catch(...) {} }
};
